...
3) Output ONLY the next handoff prompt text (PASS → next Executor, FAIL → Reviewer). No prose.
...
- `--handoff-only` prints ONLY the next prompt text (no JSON).

---

## Output Requirement (Strict)

Your entire response MUST be exactly what the Gate1 command prints (the next prompt text).
