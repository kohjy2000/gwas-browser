...
- 런타임에서 원격 보강을 켜려면(옵션), 매번 `export` 치지 말고 아래를 `ai_workflow/.env.local`에 1회만 추가:

- `GWAS_REMOTE_SEARCH=1`

### Trait search cache (phenotype/trait “caching” 복구)

현재 `data/trait_list.json`이 너무 작으면(예: 수십~수백개) 부분 문자열 검색(예: `Obes`)이 체감상 망가진다.

런타임에서 자동으로 큰 trait cache를 만들도록 지원한다(기본 ON, 테스트에서는 파일 생성 금지):

- 입력 TSV(자동 탐지):
  - `/Users/june-young/Research_Local/08_GWAS_browser/gwas_catalog_trait-mappings_r2025-09-04.tsv`
  - 또는 최신 `gwas_catalog_trait-mappings_*.tsv`
- 생성 파일(런타임 캐시, git ignore):
  - `/Users/june-young/Research_Local/08_GWAS_browser/ver_260201_toy_gwas_browser/data/trait_list.full.json`
  - `/Users/june-young/Research_Local/08_GWAS_browser/ver_260201_toy_gwas_browser/data/trait_list.full.meta.json`

원하면 명시적으로 경로 지정:

- `GWAS_TRAIT_MAPPINGS_TSV_PATH=/Users/june-young/Research_Local/08_GWAS_browser/gwas_catalog_trait-mappings_r2025-09-04.tsv`

자동 생성 끄기:

- `GWAS_TRAIT_LIST_AUTOBOOTSTRAP=0`
...
`ai_workflow/.env.local`에 아래 2줄을 넣어라(매번 export 금지):
...
### (권장) env 파일 자동 로드로 `export` 생략

아래 파일이 존재하면, 서버 + Gate 도구들이 자동으로 env를 로드한다:

- /Users/june-young/Research_Local/08_GWAS_browser/ver_260201_toy_gwas_browser/ai_workflow/.env.local

예시 파일:

- /Users/june-young/Research_Local/08_GWAS_browser/ver_260201_toy_gwas_browser/ai_workflow/.env.local.example

### ForeGenomics PGx ingest (optional, Cycle 8)
...
기존 cache가 PubMed가 비어있는 상태로 굳어버린 경우(“영원히 PubMed 없음”), 런타임에서 self-heal을 켜려면:

- `GWAS_CACHE_REQUIRE_PUBMED=1`

### (권장) GWAS cache 영속화(버전 폴더 간 공유) — Cycle 9

버전 폴더(`ver_*`)를 새로 만들면 `data/gwas_cache/`가 “사라진 것처럼” 보이고 다시 fetch하는 것처럼 느껴진다.
가장 깔끔한 해결은 캐시를 프로젝트 바깥의 공용 폴더로 빼는 것이다.

`ai_workflow/.env.local`에 아래를 추가(예시):

- `GWAS_CACHE_DIR=/Users/june-young/Research_Local/08_GWAS_browser/_shared_cache/gwas_cache`
