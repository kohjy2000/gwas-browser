...
## Cycle 10 (Phase 7) — UX/Reality Tightening Contracts

These contracts exist because “Gate2 PASS” can still feel broken:
- PubMed reference URLs must be **direct and actionable**.
- ForeGenomics PGx must be **individual-specific** (session/sample-aware).
- Chat must not answer trait-risk questions from irrelevant facts (PGx-only).

## Contract: Data_Reference_URL_Quality_v3
...
1. If `PubMed_ID` is a non-empty integer-like string, `variants[].reference` must equal:
...
## Contract: API_PGxSummary_ForeGenomics_SessionSelect_v1
...
| Path | /api/pgx-summary |
| Input | JSON: {"source": "foregenomics", "session_id": "..."} |
| Env | FOREGENOMICS_PGX_ROOT (directory containing per-sample reports) |
| Output | JSON includes deterministic `summary` and mandatory `disclaimer_tags` |

### Validation Rules

1. If `FOREGENOMICS_PGX_ROOT` is set and the session has a `sample_id`, the backend must pick the matching report for that sample_id (instead of a single global report path).
2. Two different fixture sessions (different sample_id) must produce different summaries (at least 1 differing gene/drug).
3. No network calls in contract tests.
...
## Contract: API_Chat_TraitRisk_Guard_v1
...
| Method | POST |
| Path | /api/chat |
| Input | JSON: {"message": "...", "session_id": "..."} |
| Output | JSON: {"success": bool, "answer": str, "risk_level": str, "suggested_actions": [...], "citations": [...], "disclaimer_tags": [...]} |

### Validation Rules

1. If the message mentions a trait and the session has **no GWAS facts**, chat must not output medium/high “disease risk” based only on PGx facts.
2. In this case, risk_level must be "low" and `suggested_actions` must include an `analyze_trait` action for the detected trait.
3. Contract tests do not require Ollama running; deterministic branch must satisfy this contract.
